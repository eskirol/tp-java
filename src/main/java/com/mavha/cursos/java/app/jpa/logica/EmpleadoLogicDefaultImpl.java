...
public class EmpleadoLogicDefaultImpl implements EmpleadoLogic{
    ...
    private BiPredicate<Double,Double> salarioValidoEmpresa = (mediaEmpresa,salEmpleado)-> (salEmpleado >= mediaEmpresa * 0.2) || (salEmpleado < mediaEmpresa * 1.5); //true;
    ...
    private BiPredicate<Double,Double> salarioValidoDepto = (mediaDepto,salEmpleado)-> (salEmpleado > mediaDepto * 0.8) && (salEmpleado < mediaDepto * 1.5); //true;
    ...
    public void asignarDepartamento(Integer idEmpleado, Integer idDepto) {
        ...
        Double salarioMedioDepto = depto.getEmpleados().stream().mapToDouble((Empleado emp1)->{
            return emp1.getSalarioHora();
        }).average().orElse(0.0);                        
